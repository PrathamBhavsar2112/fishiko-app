...
def predict_fn(input_data, model_dict):
    try:
        ...
        if predicted_zone not in ZONES:
            predicted_zone = 'Zone A'  # Default if invalid
        ...
        temp_range = fish_info.get('temp_range', (0, 100))
        sal_range = fish_info.get('salinity_range', (0, 100))
        if not (temp_range[0] <= temperature <= temp_range[1] and sal_range[0] <= salinity <= sal_range[1]):
            species = ['None']
